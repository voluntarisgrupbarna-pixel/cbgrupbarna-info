...
// Sèrie de 30 dies: creixement realista de club de barri
const history = [];
let followers = 3040;
for (let i = 29; i >= 0; i--) {
  const d = new Date(REF.getTime() - i*864e5);
  // creixement irregular: +2..+9/dia, algun dia pla
  const gain = [3,5,2,7,4,6,3,9,5,4,2,6,8,3,5,7,4,3,6,5,9,4,2,7,6,5,3,8,4,6][29-i];
  followers += gain;
  history.push({
    date: ymd(d), followers, following: 218, posts: 612 + Math.floor((29-i)/6),
    reach30: 41000 + gain*300, profileViews30: 5200 + gain*40,
  });
}
...
const out = {
  meta: { generatedAt: REF.toISOString(), mode:'demo', account:'@cbgrupbarna', apiVersion:'v21.0',
  ...
  profile: { username:'cbgrupbarna', name:'CB Grup Barna', biography:'🏀 Club de bàsquet base · El Clot, BCN · Des de 1965 · #somclot',
    followers_count:last.followers, follows_count:218, media_count:last.posts, website:'https://cbgrupbarna.info' },
  kpis: { followers:last.followers, followersDelta:6, following:218, posts:last.posts,
    reach30:last.reach30, profileViews30:last.profileViews30,
...
console.log('✅ demo data.json escrit');
