...
(function(){
  ...
  function showGate(){
    ...
    gate.innerHTML='<style>'
      +'#cbgb-gate{position:fixed;inset:0;z-index:99999;background:#040404;display:flex;align-items:center;justify-content:center;padding:2rem;font-family:\'Outfit\',system-ui,sans-serif}'
      ...
      +'.cbgb-logo{height:72px;width:auto;filter:drop-shadow(0 0 18px rgba(200,16,46,.45));margin-bottom:1.75rem}'
      +'.cbgb-ey{font-size:.5rem;letter-spacing:.4em;text-transform:uppercase;color:rgba(200,16,46,.8);margin-bottom:1.1rem}'
      +'.cbgb-title{font-family:\'Cormorant Garamond\',Georgia,serif;font-size:clamp(1.8rem,5vw,2.6rem);font-weight:700;color:#F2EDE6;line-height:1.1;margin-bottom:.4rem}'
      +'.cbgb-title .r{color:#C8102E;font-style:italic}'
      +'.cbgb-sub{font-family:\'Cormorant Garamond\',Georgia,serif;font-style:italic;font-size:clamp(.85rem,2.5vw,1rem);color:rgba(242,237,230,.4);margin-bottom:2.25rem}'
      +'.cbgb-form{display:flex;flex-direction:column;gap:.75rem;align-items:center}'
      +'.cbgb-input{background:transparent;border:1px solid rgba(200,16,46,.35);color:#F2EDE6;padding:.85rem 1.25rem;font-family:\'Outfit\',system-ui,sans-serif;font-size:1.1rem;letter-spacing:.35em;text-align:center;width:min(260px,85vw);outline:none;transition:border-color .25s ease}'
      +'.cbgb-input:focus{border-color:rgba(200,16,46,.8)}'
      +'.cbgb-input::placeholder{color:rgba(242,237,230,.2);letter-spacing:.15em;font-size:.75rem}'
      +'.cbgb-btn{background:#C8102E;border:none;color:#F2EDE6;padding:.85rem 2rem;font-family:\'Outfit\',system-ui,sans-serif;font-size:.68rem;font-weight:500;letter-spacing:.25em;text-transform:uppercase;cursor:pointer;transition:all .25s ease;width:min(260px,85vw)}'
      +'.cbgb-btn:hover{background:#a50d26}'
      +'.cbgb-err{color:#ff4060;font-size:.65rem;letter-spacing:.1em;display:none;margin-top:.25rem}'
      +'.cbgb-foot{margin-top:2.5rem;font-size:.5rem;letter-spacing:.2em;text-transform:uppercase;color:rgba(242,237,230,.18)}'
      +'</style>'
      +'<div class="cbgb-wrap">'
